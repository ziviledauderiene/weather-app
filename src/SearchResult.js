...
const SearchResult = ({ searchResult, setLocation, setCity, setDisplaySearchResults, setCurrentConditions, setForecastData, setLoading }) => {
  
  const getCurrentWeather = () => {
    (async ()=>{
      const response = 
        await fetch("https://foreca-weather.p.rapidapi.com/current/" + searchResult.id + "?tempunit=C&windunit=MS&lang=en", {
          "method": "GET",
          "headers": {
  ...
  const getForecast = () => {
    (async ()=>{
      const response = 
        await fetch("https://foreca-weather.p.rapidapi.com/forecast/daily/" + searchResult.id + "?alt=0&tempunit=C&windunit=MS&periods=8&dataset=full", {
          "method": "GET",
          "headers": {
            "x-rapidapi-host": "foreca-weather.p.rapidapi.com",
            "x-rapidapi-key": "047e2c187bmsh433b21ef75b8c98p1b944ajsncdd12618e935"
    ...
    "url('https://source.unsplash.com/1600x900/?" + searchResult.name + "')"; // pasirinkto miesto nuotrauka
    })();
  };
  ...
  >{searchResult.name}, {searchResult.adminArea}, {searchResult.country}</p>
)};
